...
import { useOrientation, useIsTablet } from '../../hooks/useOrientation';
import './Game.css';
...
const MobileGamePage: React.FC = () => {
  ...
  const isWideLandscape = isLandscape && isTablet;
  ...
  const renderTopHUD = () => (
    <section className="relative z-20 mt-4 mb-3 px-2 shrink-0">
  ...
  const renderPortraitFooter = () => (
    <footer ref={handRef} className="player-section game-footer-section flex-shrink-0 w-full px-2 sm:px-4 z-50" style={{ paddingBottom: 'calc(env(safe-area-inset-bottom) + 8px)' }}>
      <div className="hud-glass rounded-[32px] sm:rounded-[40px] p-2 sm:p-4 space-y-2 border-2 border-white/50 shadow-2xl relative overflow-visible">
        ...
        <div className="flex items-center gap-1.5 relative z-10">
          <div className="flex gap-1 shrink-0">
            <motion.button 
              whileTap={{ scale: 0.9 }}
              onClick={() => {
                showConfirm('Bạn chắc chắn muốn bỏ cuộc chứ?', () => {
                  g.surrenderGame();
                  g.navigate('/');
                });
              }} 
        ...
        <div id="arena-hand-area" className="relative h-[clamp(110px,18vh,220px)] sm:h-[clamp(150px,22vh,300px)] flex justify-center items-end mb-12 px-2 overflow-x-auto overflow-y-visible player-hand-wrapper">
  ...
  const renderLandscapeSidebars = () => (
    <>
      <div className="w-64 h-full p-4 flex flex-col gap-4 z-20">
         <div className="hud-glass rounded-3xl p-4 flex flex-col gap-4 border border-white/50">
            <div className="flex items-center gap-3">
              <div className="size-12 rounded-2xl bg-primary/10 flex items-center justify-center border-2 border-primary/40 relative">
                <span className="text-2xl">🐼</span>
                <div className="absolute -top-4 -right-2"> <EmoteDisplay /> </div>
              </div>
              <div>
                <p className="text-[10px] font-black text-primary/40 uppercase">BẠN</p>
                <p className="text-2xl font-black italic text-primary"><RollingNumber value={g.displayP1Score} /></p>
              </div>
            </div>
            
            <div className="flex items-center gap-3 opacity-60">
              <div className="size-12 rounded-2xl bg-slate-100 flex items-center justify-center border-2 border-primary/20 relative">
                <Bot size={24} className="text-primary/60" />
                <div className="absolute -top-4 -left-2"> <EmoteDisplay isOpponent /> </div>
              </div>
              <div>
                <p className="text-[10px] font-black text-primary/40 uppercase">{player2Name}</p>
                <p className="text-2xl font-black italic text-primary"><RollingNumber value={g.displayP2Score} /></p>
              </div>
            </div>
         </div>

         <div className="flex-1 hud-glass rounded-3xl p-2 border border-white/50 overflow-hidden flex flex-col">
            <p className="text-[9px] font-black text-primary/30 uppercase tracking-widest text-center my-2">NHẬT KÝ CHIẾN THUẬT</p>
      ...
      <div className="w-24 h-full py-4 pr-4 flex flex-col items-center gap-4 z-20">
         <div className={`w-20 h-20 rounded-full border-4 flex flex-col items-center justify-center bg-white shadow-xl shrink-0 ${g.timeLeft <= 10 ? 'border-rose-500 scale-110' : 'border-primary'}`}>
            <p className="text-[8px] font-black text-primary/40 uppercase -mb-1">GIÂY</p>
            <span className={`text-2xl font-black italic ${g.timeLeft <= 10 ? 'text-rose-500 animate-pulse' : 'text-primary'}`}>
              {g.timeLeft === 999 ? '∞' : g.timeLeft}
            </span>
         </div>
         ...
         <div className="flex-1 flex flex-col gap-2 w-full">
            <motion.button 
              whileTap={{ scale: 0.9 }}
              onClick={() => setIsRulesOpen(true)}
              className="w-full aspect-square flex items-center justify-center hud-glass rounded-2xl text-primary font-black text-2xl border-2 border-primary/10"
            >
              ?
            </motion.button>
            <motion.button 
              whileTap={{ scale: 0.9 }}
              onClick={() => setIsEmotePickerOpen(!isEmotePickerOpen)}
              className={`w-full aspect-square rounded-2xl flex items-center justify-center transition-all ${isEmotePickerOpen ? 'bg-primary text-white' : 'bg-white/80 text-primary border border-primary/20 shadow-sm'}`}
            >
              <span className="text-2xl">😊</span>
            </motion.button>
            <motion.button 
              ...
              className="w-full aspect-square flex items-center justify-center hud-glass rounded-2xl text-rose-500 border border-rose-100"
            >
              <Flag size={24}/>
            </motion.button>
         </div>
         ...
         <motion.button
            ...
            onClick={() => g.submitTurn()}
            disabled={!g.playValid || g.isProcessing}
            className={`w-full py-6 rounded-2xl font-display font-black text-[10px] uppercase tracking-widest vertical-text ${
                (g.playValid && !g.isProcessing) ? 'bg-primary text-white shadow-lg' : 'bg-gray-100 text-gray-400'
            }`}
          >
            {g.isProcessing ? '...' : 'XÁC NHẬN'}
          </motion.button>
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* Premium HUD: Score & Timer - Conditional Layout */}
        ...
        <div id="arena-interaction-zone" className={`game-arena-section flex-1 min-h-0 flex ${isWideLandscape ? 'flex-row' : 'flex-col'} overflow-hidden relative z-10`}>
          ...
          {!isWideLandscape && renderPortraitFooter()}
        </div>
        
        {/* LANDSCAPE HAND AREA (Dưới cùng khi ở mode iPad) */}
        {isWideLandscape && (
          <div className="w-full pb-6 px-4 shrink-0 flex flex-col items-center">
             <div className="flex gap-2 mb-4">
                <motion.button 
                  whileTap={{ scale: 0.95 }}
                  onClick={() => g.activateMulligan()}
                  disabled={g.hasUsedMulligan || g.placedCards.length > 0 || g.isProcessing}
                  className="px-6 py-3 hud-glass rounded-2xl text-[10px] font-black text-primary uppercase border border-primary/10 disabled:opacity-30 flex items-center gap-2"
                >
                  <RefreshCw size={16} /> <span>ĐỔI BÀI</span>
                </motion.button>
                <motion.button 
                  whileTap={{ scale: 0.95 }}
                  onClick={() => g.activateWildcard()}
                  ...
                  className="px-6 py-3 hud-glass rounded-2xl text-[10px] font-black text-primary uppercase border border-primary/10 disabled:opacity-30 flex items-center gap-2"
                >
                  <Zap size={16} fill="currentColor" className="text-amber-500" /> <span>KỸ NĂNG</span>
                </motion.button>
             </div>
             
             <div id="arena-hand-area-landscape" className="relative h-[180px] flex justify-center items-end overflow-visible">
                {g.activeHand.map((card, i) => {
                   const total = g.activeHand.length;
                   const mid = (total - 1) / 2;
                   const rotation = (i - mid) * 2; 
                   const xOffset = (i - mid) * 110; 

                   return (
                     <div key={card.id} className="absolute bottom-0" style={{ transform: `translateX(${xOffset}px)` }}>
                       <DraggableCard 
                         {...card} 
                         id={card.id}
                         layoutId={card.id}
        ...
        <AnimatePresence>
          ...
          {g.skillMode !== 'none' && (
            <motion.div 
              ...
              exit={{ opacity: 0, y: -20 }}
              className="fixed top-[calc(env(safe-area-inset-top,1rem)+4.5rem)] left-4 right-4 z-100"
            >
              <div className="bg-primary/95 backdrop-blur-xl border border-white/20 rounded-[28px] p-4 shadow-2xl flex items-center justify-between">
                <div className="flex items-center gap-3">
                  <div className="size-12 rounded-2xl bg-white/10 flex items-center justify-center text-amber-400 border border-white/10 shadow-inner">
                    {g.skillMode === 'mulligan_select' ? <RefreshCw size={24} /> : <Zap size={24} fill="currentColor" />}
                  </div>
                  <div>
                    <p className="text-[10px] font-black text-white/40 uppercase tracking-[0.2em] mb-0.5">KÍCH HOẠT NĂNG LƯỢNG</p>
                    <h3 className="text-sm font-black text-white uppercase italic leading-none mb-1">
                      {g.skillMode === 'mulligan_select' ? 'Đổi Bài Mới' : 'Thẻ Vạn Năng'}
                    </h3>
                    <p className="text-[10px] font-medium text-white/60 leading-tight max-w-[140px]">
                      {g.skillMode === 'mulligan_select' 
                        ? 'Chọn các lá bài bạn muốn thay thế để nhận bộ bài mới.' 
                        : 'Chọn 1 lá bài trên tay để chuyển hóa thành giá trị bất kỳ.'}
                    </p>
                  </div>
                </div>
                ...
                <div className="flex gap-2 shrink-0">
                  <button 
                    onClick={() => g.cancelSkill()}
                    className="px-4 py-3 bg-white/10 text-white rounded-2xl text-[10px] font-black uppercase tracking-widest active:scale-90 transition-all border border-white/5"
                  >
                    HỦY
                  </button>
                  <button 
                    onClick={() => g.skillMode === 'mulligan_select' ? g.confirmMulligan() : g.confirmWildcard()}
                    disabled={(g.skillMode === 'mulligan_select' && g.mulliganSelection.length === 0) || (g.skillMode === 'wildcard_select' && !g.pendingWildcard)}
                    className="px-5 py-3 bg-linear-to-r from-amber-400 to-amber-600 text-white rounded-2xl text-[10px] font-black uppercase tracking-widest shadow-xl shadow-amber-500/20 disabled:opacity-30 active:scale-95 transition-all"
                  >
                    XÁC NHẬN
                  </button>
                </div>
        ...
        <DragOverlay zIndex={10000} dropAnimation={performanceMode === 'ECO' ? null : { duration: 220, easing: 'cubic-bezier(0.2, 0, 0, 1.6)' }}>
            ...
            <div style={{ 
              ...
              {performanceMode === 'ECO' ? (
                ...
                <motion.div 
                  ...
                  animate={{ 
                    scale: performanceMode === 'ULTRA' ? 1.35 : 1.15, 
                    rotate: performanceMode === 'ULTRA' ? 3 : 1,
                    y: performanceMode === 'ULTRA' ? -8 : -4
                  }}
                  transition={{ type: 'spring', stiffness: 500, damping: 30 }}
                  style={{ 
                    filter: performanceMode === 'ULTRA' 
