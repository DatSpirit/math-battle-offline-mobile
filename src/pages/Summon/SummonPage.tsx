...
const SummonPageMobile: React.FC = () => {
  ...
  // Reduce decoys for ECO mode
  const decoys = isEco ? HYPERSPACE_DECOYS.slice(0, 4) : HYPERSPACE_DECOYS;

  if (!showContent) {
    return <div className="h-full w-full bg-[#fcfae4]" />;
  }

  return (
    <div className="summon-page mobile flex flex-col h-full overflow-hidden relative pb-[env(safe-area-inset-bottom)]">
      ...
      >
        <div className="absolute inset-0 bg-[#fcfae4]/40 backdrop-blur-[1px]"></div>
      </div>
      ...
      <div className="relative z-10 flex-1 flex flex-col items-center p-6 summon-mobile-container">
        <header className="text-center mb-10 mt-12">
          ...
          >
             <span className="text-amber-600/50 text-[10px] font-black uppercase tracking-[0.4em] mb-2 block">ANCIENT KNOWLEDGE</span>
             <h1 className="text-5xl font-black text-on-surface uppercase italic leading-none tracking-tighter mb-4">THƯ VIỆN SỐ</h1>
             <div className="h-0.5 w-24 bg-linear-to-r from-transparent via-amber-600/50 to-transparent mx-auto" />
          </motion.div>
        </header>
        ...
        <div className="flex-1 flex flex-col justify-center w-full max-w-sm gap-4 summon-bookshelf-mobile">
           ...
           <BookItem 
             ...
             cost={250} 
             currency="gem" 
             icon={<ZapIcon size={24} className="text-amber-300 drop-shadow-[0_0_8px_rgba(252,211,77,0.8)]" />} 
           ...
           <BookItem 
             ...
             cost={100} 
             currency="gem" 
             icon={<SparklesIcon size={24} className="text-blue-200 drop-shadow-[0_0_8px_rgba(191,219,254,0.8)]" />} 
           ...
           <BookItem 
             ...
             cost={200} 
             currency="gold" 
             icon={<BookOpenIcon size={24} className="text-white drop-shadow-[0_0_8px_rgba(255,255,255,0.8)]" />} 
        ...
        <div className="mt-auto pb-10 px-6 text-center librarian-quote-mobile rounded-2xl py-4">
           <p className="text-on-surface/60 font-medium italic text-[10px] tracking-widest leading-relaxed uppercase">
      ...
      <AnimatePresence>
        ...
        {s.isSolving && s.currentProblem && (
          <MathChallengeModal key="math-modal" s={s} />
        )}

        {s.isWormholeActive && (
          <HyperspaceEffect key="wormhole" decoys={decoys} onSkip={s.skipWormhole} />
        )}

        {s.isOpening && (
          <GachaRevealOverlay key="gacha-reveal" cards={s.openedCards} onClose={() => { s.setIsOpening(false); setSelectedTier(null); }} />
        )}
        
        {s.rewardModal?.isOpen && (
          <RewardClaimModal key="reward-claim" rewards={s.rewardModal.rewards} onComplete={() => s.setRewardModal(null)} />
        )}
        
        {s.showConfetti && !isEco && (
          <Confetti key="confetti" width={window.innerWidth} height={window.innerHeight} recycle={false} numberOfPieces={isEco ? 0 : 200} gravity={0.15} style={{ zIndex: 1200 }} />
        )}
      </AnimatePresence>
    </div>
  );
...
const BookItem: React.FC<{
  ...
  <motion.div 
    whileTap={{ scale: 0.98 }}
    onClick={onClick}
    className={`flex items-center bg-white/40 border border-black/5 rounded-[28px] p-4 gap-5 shadow-2xl active:bg-black/5 transition-colors backdrop-blur-xl perf-blur-off`}
  >
    ...
    <div className="flex-1">
      <h3 className="text-on-surface font-black uppercase italic text-xs tracking-wide mb-1">{title}</h3>
...
const SummonSelectionModal: React.FC<{ tier: SummonTier; s: SummonLogic; onClose: () => void }> = ({ tier, s, onClose }) => (
  <div className="fixed inset-0 z-6000 flex items-center justify-center p-6">
    ...
    <motion.div 
      ...
      className="summon-selection-modal relative rounded-[48px] p-8 w-full max-w-sm text-center border border-white/10"
    >
       <div className="size-16 bg-primary/10 rounded-full flex items-center justify-center mx-auto mb-4 border border-primary/20 text-primary">
          <ZapIcon size={32} />
       </div>
       <h2 className="text-3xl font-black text-on-surface uppercase italic mb-8 tracking-tighter">Giao Ước</h2>
       
       <div className="flex flex-col gap-4">
          <button onClick={() => s.initiateSummon(tier, 'x1')} className="summon-option-btn x1">
             <span className="btn-label text-on-surface/60 font-black uppercase text-[10px] tracking-widest">THUÊ 1 QUYỂN</span>
             <div className="flex items-center gap-2">
                {!s.freeSummonsUsed[tier] ? (
                  ...
                  <>
                    <span className="font-black text-on-surface text-xl">{tier === 'summon_normal' ? '200' : tier === 'summon_rare' ? '100' : '250'}</span>
                    {tier === 'summon_normal' ? <CoinIcon size={16} className="text-amber-600" /> : <GemIcon size={16} className="text-blue-600" />}
                  </>
                )}
             </div>
          </button>
          <button onClick={() => s.initiateSummon(tier, 'x10')} className="summon-option-btn x10">
             <div className="flex flex-col items-start leading-none">
                <span className="btn-label font-black uppercase text-[10px] tracking-widest">THUÊ 10 QUYỂN</span>
                <span className="text-[9px] font-black text-white/50 uppercase tracking-widest mt-1">GIẢM 10%</span>
             </div>
             <div className="flex items-center gap-2">
                <span className="font-black text-white text-xl">{tier === 'summon_normal' ? '1,800' : tier === 'summon_rare' ? '900' : '2,250'}</span>
                {tier === 'summon_normal' ? <CoinIcon size={16} className="text-white" /> : <GemIcon size={16} className="text-white" />}
             </div>
          </button>
       </div>
       <button onClick={onClose} className="mt-8 text-on-surface/20 text-[10px] font-black uppercase tracking-[0.2em] hover:text-on-surface/40 transition-colors">QUAY LẠI THƯ VIỆN</button>
    </motion.div>
  </div>
);
...
const MathChallengeModal: React.FC<{ s: SummonLogic }> = ({ s }) => (
  <div className="fixed inset-0 z-7000 flex items-center justify-center p-6">
    ...
    <motion.div 
      ...
      className="summon-math-challenge relative p-10 w-full max-w-sm text-center"
    >
      <div className="mb-8">
         <div className="bg-primary/10 text-primary px-4 py-1.5 rounded-full text-[10px] font-black uppercase tracking-[0.3em] w-fit mx-auto mb-6 border border-primary/20">GIẢI MÃ PHONG ẤN</div>
         <h2 className="math-question-text">{s.currentProblem?.question}</h2>
      </div>

      <div className="math-timer-container">
...
const HyperspaceEffect: React.FC<{ decoys: typeof HYPERSPACE_DECOYS; onSkip: () => void }> = ({ decoys, onSkip }) => (
